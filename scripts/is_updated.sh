...
latest_tag="$(git ls-remote --tags "https://github.com/$current_repo" | awk -F/ '$3 ~ /^'"$tag_prefix"'[0-9]+(\.[0-9]+){1,2}$/{gsub(/^'"$tag_prefix"'/, "", $3); print $3}' | sort --version-sort --reverse | head -n1)"
